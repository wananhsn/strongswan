	...
	private_openssl_ec_diffie_hellman_t *this, chunk_t value)
{
	...
	pub = EVP_PKEY_new();
	if (!pub || EVP_PKEY_copy_parameters(pub, this->key) <= 0)
	{
		goto error;
	}
	...
	if (EVP_PKEY_set1_tls_encodedpoint(pub, value.ptr, value.len) <= 0)
	{
...
openssl_ec_diffie_hellman_t *openssl_ec_diffie_hellman_create(diffie_hellman_group_t group)
{
	...
	if (!this->key || !EVP_PKEY_set1_EC_KEY(this->key, key))
	{
		destroy(this);
		return NULL;
	}
